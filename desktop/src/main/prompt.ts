...
export function buildSystemPrompt(customInstructions: string, features: { calendar: boolean; calendarAutoSave: boolean } = { calendar: false, calendarAutoSave: false }): string {
  const parts = [
    ...
    ...(features.calendar
      ? [
          `- The user's real calendar (Google, iCloud or Outlook, shown in the Notion Calendar app) is separate from Notion databases and is handled by the calendar tools: call calendar_status first (the app must be open), then calendar_create_event with an ISO 8601 start. ${features.calendarAutoSave ? "It saves automatically; ask the user to glance at the app to confirm the event." : "It leaves the new event open in Notion Calendar for the user to check and press Enter to save - tell them that explicitly."} It is keystroke automation and cannot read events back, so never claim an event exists until the user confirms. When the user says "my calendar" and no Notion database is an obvious match, use the calendar tools.`,
        ]
      : []),
