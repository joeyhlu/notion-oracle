...
export function buildSystemPrompt(customInstructions: string): string {
  const parts = [
    ...
    `- Editing a page: append_to_page adds Markdown to the end of a page; update_page changes properties. To rewrite existing content, quote the replacement in your reply and explain that in-place block editing is not available yet, unless the user asked you to append.`,
